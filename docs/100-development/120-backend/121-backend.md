...
To summarize: A user request first gets intercepted by a middleware, which can for example check if a user is authenticated. The routing component then determines the action that should be performed based on the URL. Usually, the routing delegates to a function which is defined in a controller.<br>
