...
resource "hcloud_server" "monitoring" {
  ...
  public_net {
    ipv4_enabled = true
    ipv4         = hcloud_primary_ip.monitoring_ipv4.id
    ipv6_enabled = true
    ipv6         = hcloud_primary_ip.monitoring_ipv6.id
  }
  provisioner "remote-exec" {
    connection {
      host        = self.ipv4_address
  ...
  lifecycle {
    precondition {
      condition     = contains(local.hcloud.validations.server_type, var.monitoring_server_type)
      error_message = "The server type \"${var.monitoring_server_type}\" is not available in the Hetzner Cloud. Available types: ${join(", ", local.hcloud.validations.server_type)}"
    }
  }
}
